...
extern crate trtc;
use trtc::tuples::{point, vector, tuple, magnitude, normalize};
use trtc::tuples::{Point, Vector, Tuple};
...
mod tuples_steps {
    ...
    use cucumber::steps;
    use typename::TypeName;
    use trtc::tuples::{vector, tuple, point, magnitude, normalize};
    use trtc::tuples::{Vector, Tuple, Point};
    steps!(::MyWorld => {
        ...
        then regex r#"normalize\(vnorm123\) == approximately vector\(([\-0-9.]+), ([\-0-9.]+), ([\-0-9.]+)\)"# (f64, f64, f64) | world, x, y, z, _step | {
            // println!("{:?}", normalize(&world.bvec));
            // println!("{:?}", vector(x, y, z));
            // println!("{:?}", normalize(&world.bvec) == vector(x, y, z));
            assert!(normalize(&world.bvec) == vector(x, y , z))
        };
    });
